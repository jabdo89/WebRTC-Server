...
function negotiate() {
  ...
  var callurl =
    ...
    peerId +
    "&url=" +
    encodeURIComponent("Pollo Loco Gomez Morin 18");
  return pc
    .createOffer()
    .then(function (offer) {
      return pc.setLocalDescription(offer, function () {
        fetch(callurl, {
          method: "POST",
          ...
          .then((response) => {
            ...
            pc.setRemoteDescription(descr, function () {
              ...
              fetch(
                ...
                .then((response) => response.json())
                .then((response) => {
                  console.log("candidate: " + JSON.stringify(response));
                  if (response) {
                    for (var i = 0; i < response.length; i++) {
                      var candidate = new RTCIceCandidate(response[i]);
                      ...
                      pc.addIceCandidate(
                        ...
                        function (error) {
                          console.log(
                            "addIceCandidate error:" + JSON.stringify(error)
                          );
                        }
                      );
                    }
    ...
    .then(function (answer) {
      ...
      return pc.setRemoteDescription(pc.currentRemoteDescription);
    })
    .catch(function (e) {
      alert(e);
    });
